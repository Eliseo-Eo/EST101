$(document).ready(function () {
  ...
  $('#menu').on('click', 'li', function () {
    ...
    // Cerrar el menú si estamos en dispositivos pequeños
    if ($(window).width() <= 768) {
...
function cargarGaleria() {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    window.mostrarGrupos = function (turno) {
      // Usamos jQuery para las animaciones
      $('#grupos-matutino, #grupos-vespertino').slideUp(); // Ocultamos ambos
      ...
      if (turno === 'matutino') {
        $('#grupos-matutino').slideDown(); // Mostramos matutino
      } else if (turno === 'vespertino') {
        $('#grupos-vespertino').slideDown(); // Mostramos vespertino
      }
    };
    ...
    function mostrarMaterias(turno, grado) {
      ...
      $(`#materias-matutino-1, #materias-matutino-2, #materias-matutino-3`).slideUp();
      $(`#materias-vespertino-1, #materias-vespertino-2, #materias-vespertino-3`).slideUp();
      // Ahora, mostramos solo las materias del grado seleccionado
      $(`#materias-${turno}-${grado}`).slideDown(); // Mostramos el grado seleccionado
    }
